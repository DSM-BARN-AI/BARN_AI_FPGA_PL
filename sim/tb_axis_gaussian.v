...
module tb_axis_gaussian;
    ...
    axis_gaussian #(.H_ACTIVE(H), .V_ACTIVE(V)) dut (
        .aclk          (clk),
        .aresetn       (rstn),
    ...
    always @(posedge clk) if (rstn) begin
        ...
        if (m_tvalid && m_tready) begin
            ...
            op = op + 1; cyc_end = cyc; last_prog = cyc;
        end
        if (ip >= NPIX && !s_tvalid && op < NPIX && (cyc - last_prog) > 5000) begin
            $display("[%0t] R4 FAIL: DRAIN STUCK - output %0d/%0d", $time, op, NPIX);
    ...
    initial begin
        ...
        $readmemh("gauss_in.hex",  mem_in);
        $readmemh("gauss_exp.hex", mem_exp);
        fd = $fopen("gauss_out.hex", "w");
        $dumpfile("tb_axis_gaussian.vcd");
        $dumpvars(0, tb_axis_gaussian);
