    ...
    always @(posedge aclk) begin
        if (!aresetn) begin // 리셋 시 초기화
            ...
            case (pattern_reg)  // 패턴 선택에 따라 출력 데이터 결정
                2'b00: begin    // Color Bar
                    case (bar_index)
                        3'd0: m_axis_tdata <= 24'hFFFFFF;    // 흰색
                        3'd1: m_axis_tdata <= 24'hFFFF00;    // 노란색
                        3'd2: m_axis_tdata <= 24'h00FFFF;    // 시안
                        3'd3: m_axis_tdata <= 24'h00FF00;    // 초록색
                        3'd4: m_axis_tdata <= 24'hFF00FF;    // 마젠타
                        3'd5: m_axis_tdata <= 24'hFF0000;    // 빨간색
                        3'd6: m_axis_tdata <= 24'h0000FF;    // 파란색
                        default: m_axis_tdata <= 24'h000000; // 검은색
                    endcase
                end
